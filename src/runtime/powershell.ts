...
export function friendlyError(raw: string): string {
  ...
  const preferred =
    normalized.find((line) => /exception setting|cannot set|属性|property|error/i.test(line)) ||
    normalized.find((line) => !/^\+|^at line:|^categoryinfo:|^fullyqualifiederrorid:/i.test(line)) ||
    "";
