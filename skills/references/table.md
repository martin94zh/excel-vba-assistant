...
- `create`: Create NEW table from a range (requires `sheet_name`, `table_name`, `range_address`). Pass `table_style` here to style at creation time.
...
- `append`: Add rows to existing table (requires `rows` or `rows_file` parameter)
...
- Forgetting `has_headers` parameter when creating tables from headerless data

**Server-specific quirks**:

- Style parameter is overloaded: table style name OR total function (context-dependent)
- `csv_data` parameter: dedicated parameter for append action (CSV format: comma-separated, newline-separated rows)
- `visible_only` parameter only applies to get-data action
