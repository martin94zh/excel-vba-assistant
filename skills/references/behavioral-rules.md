...
These rules ensure efficient and reliable Excel automation. AI assistants should follow these guidelines when executing Excel operations via the embedded `excel-mcp` server.
...
When starting a multi-step task, **ask the user** whether they want Excel visible or hidden.
...
```
1. file(action: 'open', path: '...')  → session_id
