...
- `set-data-labels`: Configure data labels (position, show_value, show_category, show_percentage, show_series_name, show_legend_key)

### Axis Formatting

- `get-axis-scale`: Get min, max, major_unit, minor_unit, and auto flags
...
- `set-series-format`: Configure markers (style, size, foreground_color, background_color)
...
```
1. chart(create-from-range) → chart_name
...
5. chart_config(set-data-labels, position="OutsideEnd", show_value=true)
```
