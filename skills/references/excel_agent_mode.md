...
Excel MCP's Agent Mode lets users watch AI operations happen in real-time. Instead of hidden automation, users see Excel respond to commands live.

## When to Offer Agent Mode

**Always ask the user** at session start whether they want Excel visible or hidden. Present two clear choices:
...
- User says "show me", "let me watch", "I want to see it" → Show immediately
...
When starting a session, present the visibility choice so the user can pick:
