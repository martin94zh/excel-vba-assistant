...
```
pivottable_calc(create-calculated-field, field_name="Revenue", formula="=Quantity*UnitPrice")
pivottable_field(add-value-field, field_name="Revenue", aggregation_function="Sum")
```
...
datamodel(create-measure, measure_name="Revenue", dax_formula="SUMX(Sales, Sales[Quantity]*Sales[UnitPrice])")
...
| External | `create` with source_range | NO |
...
1. Add Row fields: `pivottable_field(add-row-field, field_name="Region")`
2. Add Column fields: `pivottable_field(add-column-field, field_name="Year")`
3. Add Value fields: `pivottable_field(add-value-field, field_name="Amount", aggregation_function="Sum")`
4. Add filters: `pivottable_field(add-filter-field, field_name="Status")`
5. **Refresh to update display**: `pivottable(refresh, pivot_table_name="...")`
...
range(set-formula, sheet_name="Sales", range_address="I2", formula="=[@Quantity]*[@UnitPrice]")
pivottable(create-from-table, source_table_name="SalesTable", ...)
pivottable_field(add-value-field, field_name="Revenue", aggregation_function="Sum")  # Works!

# Option 2: Use Data Model (RECOMMENDED)
table(add-to-data-model, table_name="SalesTable")
datamodel(create-measure, measure_name="Revenue", dax_formula="SUMX(SalesTable, SalesTable[Quantity]*SalesTable[UnitPrice])")
pivottable(create-from-datamodel, ...)  # Measure automatically available
```
...
The `layout_style` parameter controls PivotTable appearance:
