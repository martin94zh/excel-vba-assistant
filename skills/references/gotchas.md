...
Default timeout for `powerquery(action: 'refresh')` is **30 minutes**. Power Query operations querying large datasets may exceed the default 5 minutes in some configurations.

**Symptoms:** Operation appears to succeed but data doesn't load, or timeout error.
...
Set `timeout_seconds` to match your expected query duration (1800 = 30 min).
...
**When this matters:** Only if you're building your own multi-threaded host. MCP Server handles this automatically.
...
When Power Query returns dates, they come back as **OLE date numbers** (e.g., 45300 for 2024-01-01). Excel handles conversion automatically in cells, but if you're reading these values programmatically, parse them accordingly.
