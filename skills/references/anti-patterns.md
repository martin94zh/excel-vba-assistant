...
| Charts | `chart_config(action:'set-style', style_number: 1-48)` | `range_format` |
...
If you already have a session_id, use it. Do not rediscover:

```
CORRECT: Use the session_id you already have
Error: "session expired"
→ file(action: 'open', path: original_path)  ← Re-open once, get new session_id
→ Continue with the new session_id immediately
```
