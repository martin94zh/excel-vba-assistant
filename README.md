...
1. 下载 `excel-vba-assistant-0.6.0.vsix`
...
早期版本使用 `node dist/mcp-server.js` 启动 MCP Server，因此依赖用户本机 Node.js。v0.6.0 起改用 `pkg` 打包为独立可执行文件，不再依赖外部 Node.js。
...
```powershell
npm run package
# 生成 excel-vba-assistant-0.6.0.vsix
```
